...
-- 3. Liste des réservations avec client, terrain et créneau
SELECT
    r.id_reservation,
    c.nom              AS Client,
...
-- 4. Liste des paiements avec le client associé
SELECT
    p.id_paiement,
    c.nom             AS Client,
    p.date_paiement,
...
-- 6. Liste des équipes avec leur créateur
SELECT
    e.id_equipe,
    e.nom_equipe,
    e.niveau,
...
-- 7. Liste des joueurs par équipe
SELECT
    e.nom_equipe,
    j.nom        AS Joueur,
    j.prenom,
...
-- 8. Liste des matchs avec les équipes et scores
SELECT
    m.id_match,
